...
def sample_and_group(npoint, radius, nsample, xyz, points, tnet_spec=None, knn=False, use_xyz=True):
    ...
    if points is not None:
        ...
        #grouped_points = tf.Print(grouped_points, [grouped_points[0, :, :, :]], 'grouped points: ', 10, 2000)
        if use_xyz:
...
def pointnet_sa_module(xyz, points, npoint, radius, nsample, mlp, mlp2, group_all, is_training, bn_decay, scope, bn=True, pooling='max', tnet_spec=None, knn=False, use_xyz=True):
    ...
    with tf.variable_scope(scope) as sc:
        ...
        for i, num_out_channel in enumerate(mlp):
            #new_points = tf.Print(new_points, [new_points], 'new points:', 1, 18)
            new_points = tf_util.conv2d(new_points, num_out_channel, kernel_size=[1,1],
                                        padding='VALID', stride=[1,1],
                                        bn=bn, is_training=is_training,
                                        scope='conv%d'%(i), bn_decay=bn_decay)
            # shp = new_points.shape
            # new_points = tf.reshape(new_points, [new_points.shape[0], -1])
            # new_points = tf_util.fully_connected(new_points, num_out_channel, 'fc-mlp_%d' % i,
            #                                      use_xavier=True,
            #                                      bn=bn, is_training=is_training,
            #                                      bn_decay=bn_decay)
            # new_points = tf.reshape(new_points, [shp[0], shp[1], shp[2], num_out_channel])
        if pooling=='avg':
